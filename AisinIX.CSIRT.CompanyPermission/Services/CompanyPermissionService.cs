...
using AisinIX.CSIRT.CompanyPermission.DBAccessors;
...
namespace AisinIX.CSIRT.CompanyPermission.Services
{
    ...
    public class CompanyPermissionService : ICompanyPermissionService
    {
        ...
        public async Task<IEnumerable<CompanyPermissionModel>> GetAllCompanyPermissionAsync()
        {
        ...
        public async Task<bool> InsertCompanyPermissionArrayAsync(IEnumerable<CompanyPermissionModel> permissions)
        {
            ...
            foreach (var permission in permissions)
            {
                if (string.IsNullOrWhiteSpace(permission.ownCompanyCode1))
                    throw new ArgumentException("自社コード1は必須です。", nameof(CompanyPermissionModel.ownCompanyCode1));
                    
                if (string.IsNullOrWhiteSpace(permission.ownCompanyCode2))
                    throw new ArgumentException("自社コード2は必須です。", nameof(CompanyPermissionModel.ownCompanyCode2));
                    
                if (string.IsNullOrWhiteSpace(permission.viewCompanyCode1))
                    throw new ArgumentException("閲覧先会社コード1は必須です。", nameof(CompanyPermissionModel.viewCompanyCode1));
                    
                if (string.IsNullOrWhiteSpace(permission.viewCompanyCode2))
                    throw new ArgumentException("閲覧先会社コード2は必須です。", nameof(CompanyPermissionModel.viewCompanyCode2));
                    
                if (string.IsNullOrWhiteSpace(permission.applicantCompanyCode1))
                    throw new ArgumentException("申請元会社コード1は必須です。", nameof(CompanyPermissionModel.applicantCompanyCode1));
                    
                if (string.IsNullOrWhiteSpace(permission.applicantCompanyCode2))
                    throw new ArgumentException("申請元会社コード2は必須です。", nameof(CompanyPermissionModel.applicantCompanyCode2));
                    
                if (string.IsNullOrWhiteSpace(permission.registUser))
                    throw new ArgumentException("登録ユーザーは必須です。", nameof(CompanyPermissionModel.registUser));
