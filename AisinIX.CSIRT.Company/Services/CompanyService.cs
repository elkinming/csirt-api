...
using AisinIX.CSIRT.Company.DBAccessors;
...
namespace AisinIX.CSIRT.Company.Services
{
    public class CompanyService : ICompanyService
    {
        ...
        public async Task<bool> InsertCompanyArrayAsync(IEnumerable<CompanyModel> companies)
        {
            ...
            foreach (var company in companies)
            {
                if (string.IsNullOrWhiteSpace(company.companyCode1))
                    throw new ArgumentException("会社コード1は必須です。", nameof(CompanyModel.companyCode1));
                    
                if (string.IsNullOrWhiteSpace(company.companyCode2))
                    throw new ArgumentException("会社コード2は必須です。", nameof(CompanyModel.companyCode2));
                    
                if (string.IsNullOrWhiteSpace(company.companyType))
                    throw new ArgumentException("会社種別は必須です。", nameof(CompanyModel.companyType));
                    
                if (string.IsNullOrWhiteSpace(company.companyName))
                    throw new ArgumentException("会社名は必須です。", nameof(CompanyModel.companyName));
                    
                if (string.IsNullOrWhiteSpace(company.companyNameEn))
                    throw new ArgumentException("会社名（英語）は必須です。", nameof(CompanyModel.companyNameEn));
                    
                if (string.IsNullOrWhiteSpace(company.companyShortName))
                    throw new ArgumentException("会社略称は必須です。", nameof(CompanyModel.companyShortName));
                    
                if (string.IsNullOrWhiteSpace(company.groupCode))
                    throw new ArgumentException("グループコードは必須です。", nameof(CompanyModel.groupCode));
                    
                if (string.IsNullOrWhiteSpace(company.region))
                    throw new ArgumentException("地域は必須です。", nameof(CompanyModel.region));
                    
                if (string.IsNullOrWhiteSpace(company.country))
                    throw new ArgumentException("国は必須です。", nameof(CompanyModel.country));
                    
                if (string.IsNullOrWhiteSpace(company.registUser))
                    throw new ArgumentException("登録ユーザーは必須です。", nameof(CompanyModel.registUser));
